...
// ── Cover cache ───────────────────────────────────────────────────────────────
// Covers are written to disk (LOG_DIR/cover-cache) and referenced by FILE PATH in
// library.json, instead of inlining megabytes of base64 — so library.json stays small,
// saves are fast, and covers persist + reload instantly across sessions. Keyed by
// SHA1(contentId||fileName). Rooted at LOG_DIR (not app.getPath('userData')) to match
// PS4 Vault's hardcoded %APPDATA%/Roaming/PS4Vault data folder.
function coverCacheDir() {
  const d = path.join(LOG_DIR || path.join(os.homedir(), 'AppData', 'Roaming', 'PS4Vault'), 'cover-cache');
  try { if (!fs.existsSync(d)) fs.mkdirSync(d, { recursive: true }); } catch (_) {}
  return d;
}
function coverCacheKey(idOrName) {
  return crypto.createHash('sha1').update(String(idOrName || '')).digest('hex');
}
function findCachedCover(idOrName) {
  if (!idOrName) return null;
  const base = path.join(coverCacheDir(), coverCacheKey(idOrName));
  for (const ext of ['.png', '.jpg', '.jpeg']) {
    try { if (fs.existsSync(base + ext)) return base + ext; } catch (_) {}
  }
  return null;
}
function saveCachedCover(idOrName, ext, buf) {
  try { const f = path.join(coverCacheDir(), coverCacheKey(idOrName) + ext); fs.writeFileSync(f, buf); return f; }
  catch (_) { return null; }
}
// Convert a cover given as a base64 data: URL into a cached file and return its path.
// Returns the original data: URL if caching fails (cover still shows this session), or
// passes through a value that is already a path/URL.
function cacheCoverFromDataUrl(idOrName, dataUrl) {
  if (!dataUrl || typeof dataUrl !== 'string') return null;
  if (!dataUrl.startsWith('data:')) return dataUrl;
  const m = /^data:image\/(png|jpe?g);base64,(.+)$/i.exec(dataUrl);
  if (!m) return dataUrl;
  const ext = m[1].toLowerCase().startsWith('jp') ? '.jpg' : '.png';
  return saveCachedCover(idOrName, ext, Buffer.from(m[2], 'base64')) || dataUrl;
}
...
function httpsGetJson(url, redirects = 0) {
  return new Promise((resolve, reject) => {
    ...
    // Enforce HTTPS end-to-end — never fetch or follow a redirect over cleartext HTTP
    // (a MITM on the LAN could otherwise swap the update metadata/payload).
    if (!/^https:\/\//i.test(url)) return reject(new Error('Refusing non-HTTPS update URL'));
    require('https').get(url, {
      ...
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        if (!/^https:\/\//i.test(res.headers.location)) return reject(new Error('Refusing non-HTTPS redirect'));
...
function httpsDownloadFile(url, destPath, onProgress, redirects = 0) {
  return new Promise((resolve, reject) => {
    ...
    if (!/^https:\/\//i.test(url)) return reject(new Error('Refusing non-HTTPS update URL'));
    require('https').get(url, { headers: { 'User-Agent': `PS4-Vault/${VERSION}` } }, (res) => {
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        if (!/^https:\/\//i.test(res.headers.location)) return reject(new Error('Refusing non-HTTPS redirect'));
      ...
      let received = 0;
      // 'wx' (exclusive): refuse to follow/clobber a file/symlink pre-planted at the
...
ipcMain.handle('download-and-install-update', async (event, downloadUrl) => {
  ...
  // Per-run private dir instead of a fixed shared-temp path, so the payload/.bat can't
  // be pre-planted or symlink-attacked (TOCTOU) by another local user.
  const updDir  = fs.mkdtempSync(path.join(os.tmpdir(), 'ps4vault-upd-'));
  const tmpFile = path.join(updDir, `update${fileExt}`);
  ...
  try {
    ...
    if (isWin) {
      const tmpBat = path.join(updDir, 'ps4vault-updater.bat');
      const bat = [
        '@echo off',
        ...
        ')',
        `start "" "${currentExe}"`,
        // Release the running batch file's handle, then remove the whole per-run
        // update dir (payload + this script) in one shot.
        `(goto) 2>nul & rmdir /s /q "${updDir}"`,
      ].join('\r\n');
    ...
    try { fs.rmSync(updDir, { recursive: true, force: true }); } catch (_) {}
...
async function parsePkgFile(filePath) {
  ...
  try {
    ...
    return {
      ...
      region,
      pkgType,
      iconPath:    cacheCoverFromDataUrl(contentId || path.basename(filePath), iconDataUrl),
      isDuplicate: false,
      isFtp:       false,
    };
  } finally {
...
async function scanPkgs(sourceDir, sender, scanDepth) {
  ...
  try {
    ...
    // Mark duplicates by (contentId, version). Two DIFFERENT versions of the same game
    // must NOT be flagged as duplicates of each other, or the user could delete a
    // legitimate patch as a "duplicate" — only same content AND same version is a dup.
    const cidMap = new Map();
    for (const item of items) {
      if (!item.contentId) continue;
      const key = item.contentId + '|' + (item.appVer || '');
      if (!cidMap.has(key)) cidMap.set(key, []);
      cidMap.get(key).push(item);
    }
    ...
    for (const partPath of (partFiles || [])) {
      const item = {
        ...
        region: '', contentId: '', pkgType: 0, iconPath: null,
...
// ── FTP helpers ───────────────────────────────────────────────────────────────
let _ftpPatched = false;
function getFtp() {
  let ftp;
  try { ftp = require('basic-ftp'); }
  catch { throw new Error('basic-ftp not installed — run: npm install'); }
  // Force LIST instead of MLSD. Some PS4 CFW / payload FTP servers advertise MLST in
  ...
  // uses). Patch the prototype once, before any client is created, so every client
  // uses LIST and never MLSD.
  if (!_ftpPatched) {
...
async function scanPkgsFtp(cfg, sender) {
  ...
  try {
    ...
    await withFtpPool(cfg, pkgList, controller.signal, async (client, { remotePath, size }, _idx) => {
      ...
      try {
        ...
        const item = {
          ...
          region, pkgType, iconPath: cacheCoverFromDataUrl(contentId || remotePath, iconDataUrl),
    ...
    // Mark duplicates by (contentId, version) — different versions are NOT duplicates.
    const cidMap = new Map();
    for (const item of items) {
      if (!item.contentId) continue;
      const key = item.contentId + '|' + (item.appVer || '');
...
// Stream a SHA-256 of a file (4 MB chunks). Used to verify a cross-device copy
// byte-for-byte BEFORE the original is deleted on a Move, so a truncated or corrupted
// copy can never cause the source — the only remaining copy — to be lost.
...
// Copy then VERIFY (size first — cheap — then SHA-256) before returning. On a verify
// mismatch it deletes the bad destination and retries up to maxAttempts (covers a
// transient I/O glitch). Throws if it cannot produce a verified copy — callers MUST NOT
// delete the source unless this resolves.
async function copyAndVerifyFile(src, dest, progressCallback, cancelCheck, maxAttempts = 3) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (destSize !== srcSize) throw new Error(`size mismatch (${destSize}/${srcSize})`);
      if (await hashFile(src) !== await hashFile(dest)) throw new Error('checksum mismatch');
      return; // verified good
    } catch (e) {
      if (e && e.message === 'Cancelled') throw e; // user cancel — don't retry/swallow
      lastErr = e;
      await fs.promises.unlink(dest).catch(() => {});
    }
  }
  throw new Error(`copy verification failed after ${maxAttempts} attempt(s): ${lastErr && lastErr.message}`);
}
...
async function moveFileWithProgress(src, dest, progressCallback, cancelCheck) {
  ...
  try {
    ...
    if (e.code === 'EXDEV' || e.code === 'EPERM') {
      // Cross-device move: copy + VERIFY (size + checksum), then delete the source only
      // after the copy is proven good. A truncated/corrupted copy can never cause the
      // original — the only remaining copy — to be lost.
      try {
        await copyAndVerifyFile(src, dest, progressCallback, cancelCheck);
      } catch (copyErr) {
      ...
      try {
        if (isFtpSrc && !ftpDest) {
          ...
          try {
            ...
            // Reject a truncated download instead of presenting a partial file as complete.
            if (item.fileSize) {
              const got = (await fs.promises.stat(destPath)).size;
              if (got !== item.fileSize) {
                await fs.promises.unlink(destPath).catch(() => {});
                throw new Error(`download incomplete (${got}/${item.fileSize} bytes)`);
              }
            }
          } finally { try { client?.close(); } catch (_) {} }
...
ipcMain.handle('save-library', async (_e, items) => {
  try {
    // Covers live on disk in cover-cache/ and are referenced by iconPath (a small file
    // path), so library.json stays tiny instead of carrying megabytes of inline base64.
    // Strip ftpCfg (credentials), _b64key (runtime) and any legacy inline iconDataUrl.
    const slim = items.map(i => { const { ftpCfg, _b64key, iconDataUrl, ...rest } = i; return rest; });
...
ipcMain.handle('load-library', async () => {
  try {
    ...
    const normalised = items.map(item => {
      ...
      // Cover: prefer the cached file path. Migrate a legacy inline iconDataUrl (data:)
      // by writing it to the cover cache once — old libraries shrink and covers persist.
      let iconPath = item.iconPath || null;
      if (!iconPath && typeof item.iconDataUrl === 'string' && item.iconDataUrl.startsWith('data:')) {
        iconPath = cacheCoverFromDataUrl(item.contentId || item.fileName || item.filePath, item.iconDataUrl);
      }
      return {
    ...
    // ENOENT = first run / no saved library yet (expected, stay silent). Anything else —
    // corrupt JSON, permission denied, disk error — is a real fault: log it instead of
    // silently showing an empty library, so the cause is visible in the log.
...
// renderer can restore iconPath on load even if a saved path was stale or missing.
ipcMain.handle('get-cached-covers', (_e, keys) =>
  Array.isArray(keys) ? keys.map(k => { try { return findCachedCover(k) || ''; } catch (_) { return ''; } }) : []
);

// Refetch covers for local PKGs missing iconPath.
      ...
      try {
        ...
        if (parsed.iconPath) {
          ...
          if (!sender.isDestroyed())
            sender.send('cover-ready', { filePath: fp, iconPath: parsed.iconPath });
        }
      } catch (_) {}
      done++;
    ...
    for (const dir of appDirs) {
      ...
      try {
        ...
        const item = {
          ...
          region,
          pkgType:     0,
          iconPath:    cacheCoverFromDataUrl(sfoData.CONTENT_ID || contentId || dir.name, iconDataUrl),
          isDuplicate: false,
          isFtp:       true,
          isInstalled: true,
          ftpCfg:      cfg,
