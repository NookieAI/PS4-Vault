(function () {
  ...
  function coverSrc(p) {
    if (!p) return '';
    if (/^(data:|file:|https?:|blob:)/i.test(p)) return p;
  ...
  async function deleteDuplicates(){
    ...
    // Group by (identity, version), keep first of each, delete rest. Keying on version
    // too means one copy of EACH version survives — a different version is never deleted
    // as a "duplicate" of another.
    const seen=new Set(),toDelete=[];
    allItems.forEach(item=>{
      if(!item.isDuplicate) return;
      const key=(item.contentId||item.titleId||item.fileName)+'|'+(item.appVer||'');
  ...
  (async()=>{
    ...
    try {
      ...
      if (r?.ok && r.items?.length) {
        ...
        // First rehydrate iconPath from the on-disk cover cache by key — covers persist
        // there even if a saved path was stale. Then refetch (re-parse the PKG) only the
        // genuine misses. (Old libraries stored base64 inline; main migrates those on load.)
        try {
          const noCover = allItems.filter(i => !i.iconPath && !i.isFtp && !i.isInstalled && i.filePath);
          if (noCover.length && pkgApi.getCachedCovers) {
            const keys   = noCover.map(i => i.contentId || i.fileName || i.filePath);
            const covers = await pkgApi.getCachedCovers(keys);
            if (Array.isArray(covers)) {
              noCover.forEach((it, idx) => { if (covers[idx]) it.iconPath = covers[idx]; });
              applyFiltersSoon();
            }
          }
        } catch (_) {}

        // Background: stream covers back one-by-one for items still without one.
        const missing = allItems
          .filter(i => !i.iconPath && !i.isFtp && !i.isInstalled && i.filePath)
          .map(i => i.filePath);
        if (missing.length && pkgApi.refetchCovers) {
          ...
          pkgApi.onCoverReady?.(d => {
            if (d.filePath && d.iconPath) {
              const item = allItems.find(x => x.filePath === d.filePath);
              if (item) { item.iconPath = d.iconPath; coversDirty = true; applyFiltersSoon(); }
            }
            if (d.progress) {
